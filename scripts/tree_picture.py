...
def get_all_pictures_in_event(event):
    event.pictures = list()
    for pictures in models.Picture.objects.filter(event_id=event.id):
        if pictures:
        ...
        if picture:
            ...
            if os.path.exists(picture_path):
                ...
                if len(filename_bdd) > 250:
                    ...
                    p = models.Picture.objects.get(pk=picture.id)
